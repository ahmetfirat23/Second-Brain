...
const keyFn = i => `${i.title.toLowerCase().trim()}|${i.year}`;
const map = new Map();
...
for (let i = 0; i < items.length; i += BATCH) {
  ...
  try {
    const out = execSync(`cd /Users/afg/Documents/SecondBrain && npx convex run mediaList:bulkCreateInternal '${json.replace(/'/g, "'\\''")}' --no-push`, { encoding: 'utf-8' });
    const result = JSON.parse(out.trim());
    totalImported += result.ids?.length ?? 0;
    totalSkipped += result.skipped ?? 0;
    process.stdout.write(`\rBatch ${Math.floor(i/BATCH)+1}/${Math.ceil(items.length/BATCH)}: ${totalImported} imported`);
  } catch(e) {
