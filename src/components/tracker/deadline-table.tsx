...
function UrgencyBadge({ days }: { days: number }) {
  ...
  return <span className="text-xs text-[hsl(0_0%_45%)]">{days}d left</span>;
}
...
export function DeadlineTable() {
  ...
  return (
    <div>
      {showAdd ? (
        <div className="mb-4 bg-[hsl(0_0%_7%)] border border-[hsl(263_90%_60%/0.3)] rounded-xl p-4 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowAdd(false)} className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-[hsl(0_0%_12%)] text-[hsl(0_0%_60%)] text-sm">
      ...
      {items.length === 0 ? (
        <div className="text-center py-20 text-[hsl(0_0%_30%)]"><p className="text-sm">No deadlines yet. Add one above or use Brain Dump.</p></div>
      ) : (
        <div className="rounded-xl border border-[hsl(0_0%_13%)] overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[hsl(0_0%_13%)] bg-[hsl(0_0%_6%)]">
                <th className="text-left px-4 py-3 text-xs font-medium text-[hsl(0_0%_40%)] uppercase tracking-wider">Task</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-[hsl(0_0%_40%)] uppercase tracking-wider hidden sm:table-cell">Deadline</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-[hsl(0_0%_40%)] uppercase tracking-wider hidden md:table-cell">Category</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-[hsl(0_0%_40%)] uppercase tracking-wider">Status</th>
            ...
            <tbody className="divide-y divide-[hsl(0_0%_11%)]">
              {items.map((item) => {
                ...
                return (
                  <tr key={item._id} className={`group transition-colors ${isUrgent ? "bg-red-950/20 hover:bg-red-950/30" : "hover:bg-[hsl(0_0%_8%)]"}`}>
                    {isEditing ? (
                      <>
                        <td className="px-4 py-3" colSpan={3}>
                          <div className="flex flex-col sm:flex-row gap-2">
                            ...
                            <input type="date" value={editDeadline} onChange={(e) => setEditDeadline(e.target.value)} className="bg-[hsl(0_0%_10%)] border border-[hsl(0_0%_18%)] rounded-lg px-3 py-1.5 text-sm text-white outline-none [color-scheme:dark]" />
                            <select value={editCategory} onChange={(e) => setEditCategory(e.target.value as Category)} className="bg-[hsl(0_0%_10%)] border border-[hsl(0_0%_18%)] rounded-lg px-3 py-1.5 text-sm text-white outline-none">
                              {CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
                            </select>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-1">
                            ...
                            <button onClick={() => setEditingId(null)} className="p-1.5 rounded-md hover:bg-[hsl(0_0%_12%)] text-[hsl(0_0%_40%)]"><X className="w-3.5 h-3.5" /></button>
                          </div>
                        </td>
                      </>
                    ) : (
                      <>
                        <td className="px-4 py-3">
                          ...
                          <div className="sm:hidden mt-1 text-xs text-[hsl(0_0%_45%)]">{item.deadline}</div>
                        </td>
                        <td className="px-4 py-3 text-[hsl(0_0%_55%)] hidden sm:table-cell">{format(parseISO(item.deadline), "MMM d, yyyy")}</td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity justify-end">
                            <button onClick={() => startEdit(item._id, item.task, item.deadline, item.category)} className="p-1.5 rounded-md hover:bg-[hsl(0_0%_12%)] text-[hsl(0_0%_40%)] hover:text-white"><Pencil className="w-3.5 h-3.5" /></button>
                            <button onClick={() => { if (confirm("Delete this deadline?")) removeDeadline({ id: item._id }); }} className="p-1.5 rounded-md hover:bg-red-900/40 text-[hsl(0_0%_40%)] hover:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>
                          </div>
                        </td>
                      </>
                    )}
