...
function VaultRow({ item }: { item: VaultItem }) {
  ...
  return (
    <div className="group relative bg-[hsl(0_0%_7%)] border border-[hsl(0_0%_12%)] rounded-xl p-4 hover:border-[hsl(0_0%_18%)] transition-all">
      {editing ? (
        <div className="space-y-2">
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Title" className="w-full bg-[hsl(0_0%_10%)] border border-[hsl(0_0%_18%)] rounded-lg px-3 py-1.5 text-sm text-white outline-none" />
          <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="URL" className="w-full bg-[hsl(0_0%_10%)] border border-[hsl(0_0%_18%)] rounded-lg px-3 py-1.5 text-sm text-white outline-none" />
          <div className="flex items-center gap-3">
            <label className="text-xs text-[hsl(0_0%_40%)]">Urgency:</label>
            {[1, 2, 3, 4, 5].map((n) => (
              <button key={n} onClick={() => setUrgency(n)}
                className={`w-7 h-7 rounded-lg text-xs font-medium transition-colors ${n === urgency ? "bg-[hsl(263_90%_60%)] text-white" : "bg-[hsl(0_0%_12%)] text-[hsl(0_0%_50%)] hover:bg-[hsl(0_0%_16%)]"}`}>{n}</button>
            ))}
          </div>
          <div className="flex gap-2 pt-1">
            ...
            <button onClick={handleCancel} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[hsl(0_0%_12%)] text-[hsl(0_0%_60%)] text-xs"><X className="w-3 h-3" /> Cancel</button>
          </div>
        </div>
      ) : (
        <div className="flex items-start gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-start gap-2 mb-1">
              ...
              <ExternalLink className="w-3.5 h-3.5 text-[hsl(0_0%_35%)] shrink-0 mt-0.5" />
            </div>
            <p className="text-xs text-[hsl(0_0%_35%)] truncate">{domain}</p>
          </div>
          ...
          <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <button onClick={() => { if (confirm(`Delete "${item.title}"?`)) removeItem({ id: item._id }); }} className="p-1.5 rounded-md hover:bg-red-900/40 text-[hsl(0_0%_40%)] hover:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>
          </div>
        </div>
      )}
    </div>
...
export function VaultList() {
  ...
  return (
    <div>
        ...
        {!showAdd && !showBulk && (
          <>
            <button onClick={() => setShowAdd(true)}
              className="flex items-center gap-2 text-sm text-[hsl(0_0%_40%)] hover:text-white border border-dashed border-[hsl(0_0%_18%)] hover:border-[hsl(0_0%_30%)] rounded-xl px-4 py-3 flex-1 transition-all">
      ...
      {showAdd && (
        <div className="mb-4 bg-[hsl(0_0%_7%)] border border-[hsl(263_90%_60%/0.3)] rounded-xl p-4 space-y-3">
          <h3 className="text-sm font-medium text-white">Add Link</h3>
          <input autoFocus value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder="Title" className="w-full bg-[hsl(0_0%_10%)] border border-[hsl(0_0%_18%)] rounded-lg px-3 py-2 text-sm text-white placeholder:text-[hsl(0_0%_30%)] outline-none" />
          <input value={newUrl} onChange={(e) => setNewUrl(e.target.value)} placeholder="https://…" className="w-full bg-[hsl(0_0%_10%)] border border-[hsl(0_0%_18%)] rounded-lg px-3 py-2 text-sm text-white placeholder:text-[hsl(0_0%_30%)] outline-none" />
          <div className="flex items-center gap-3">
            <label className="text-xs text-[hsl(0_0%_40%)]">Urgency:</label>
            {[1, 2, 3, 4, 5].map((n) => (
              <button key={n} onClick={() => setNewUrgency(n)}
                className={`w-7 h-7 rounded-lg text-xs font-medium transition-colors ${n === newUrgency ? "bg-[hsl(263_90%_60%)] text-white" : "bg-[hsl(0_0%_12%)] text-[hsl(0_0%_50%)] hover:bg-[hsl(0_0%_16%)]"}`}>{n}</button>
            ))}
          </div>
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowAdd(false)} className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-[hsl(0_0%_12%)] text-[hsl(0_0%_60%)] text-sm"><X className="w-3.5 h-3.5" /> Cancel</button>
          </div>
        </div>
      )}
      ...
      {showBulk && (
        <div className="mb-4 bg-[hsl(0_0%_7%)] border border-[hsl(263_90%_60%/0.3)] rounded-xl p-4 space-y-3">
          ...
          <p className="text-xs text-[hsl(0_0%_40%)]">Paste one URL per line. Title is extracted automatically.</p>
          <textarea value={bulkText} onChange={(e) => setBulkText(e.target.value)} rows={6} placeholder={"https://example.com\nhttps://another.site\n…"}
            className="w-full bg-[hsl(0_0%_10%)] border border-[hsl(0_0%_18%)] rounded-lg px-3 py-2 text-sm text-white placeholder:text-[hsl(0_0%_25%)] outline-none resize-none font-mono" />
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowBulk(false)} className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-[hsl(0_0%_12%)] text-[hsl(0_0%_60%)] text-sm"><X className="w-3.5 h-3.5" /> Cancel</button>
          </div>
        </div>
      )}

      {items.length === 0 && !showAdd && !showBulk ? (
        <div className="text-center py-20 text-[hsl(0_0%_30%)]"><p className="text-sm">No links yet. Add one above or paste a brain dump with URLs.</p></div>
      ) : (
